...
public class fisCon_Sprint11 extends Configuration {
	...
	public void FISC_1048_obligors() {
		...
		String IssueResourceURi = baseURI + "/v1/issues/96592767";
		...
		String obligorslink = res.path("data.relationships.obligors.links.related");
		...
		Response res1 = given().header("Authorization", AuthrztionValue).header("X-App-Client-Id", XappClintIDvalue)
				.header("accept", acceptValue).header("content", contentValue).contentType("application/vnd.api+json")
				.when().get(obligorslink).then().statusCode(200).extract().response();
	...
	public void FISC_1052_Issue_Analystattributes() {
	...
	public void FISC_1129_TransactionSecurity() {
		...
		String transactionSecurityLink = res.path("data.relationships.transactionSecurities.links.related");

		System.out.println(transactionSecurityLink);

		Response res1 = given().header("Authorization", AuthrztionValue).header("X-App-Client-Id", XappClintIDvalue)
				.header("accept", acceptValue).header("content", contentValue).contentType("application/vnd.api+json")
				.when().get(transactionSecurityLink).then().statusCode(200).extract().response();
	...
	public void FISC_1134() throws IOException {
		...
		Response fieldResponse = given().header("Authorization", AuthrztionValue)
				...
				.body(containsString("China Minsheng Banking Corp., Ltd.")).body(containsString("Bank of Beijing"))
				.extract().response();
