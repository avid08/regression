...
public class FisCon_Sprint11 extends Configuration {
	...
	@Test
	 ...
	 public void FISC_1052_Issue_Analstattributes (){
	  ...
	  Response res = given().header("Authorization", AuthrztionValue).header("X-App-Client-Id", XappClintIDvalue)
				...
				.when().get(IssueResourceURi).then()
				.statusCode(200)
				.body(containsString("obligors"))
				.body(containsString("transactionSecurities"))
				.body(containsString("originalRatingDate"))
				.body(containsString("analysts"))
				.body(containsString("id"))
				.body(containsString("roleId"))
	...
	public void FISC_1048_obligors(){		
		 ...
		 /* String obligorslink = res.path("data[0].relationships.obligors.links.related");
...
@Test
	...
	public void FISC_1129_TransactionSecurity(){
		 ...
		 /* String transactionSecurityLink = res.path("data[0].relationships.transactionSecurities.links.related");
		  
		  System.out.println(transactionSecurityLink);
		  */
		 /* Response res1 = given().header("Authorization", AuthrztionValue).header("X-App-Client-Id", XappClintIDvalue)
					.header("accept", acceptValue).header("content", contentValue).contentType("application/vnd.api+json")
					.when().get(transactionSecurityLink).then()
					.statusCode(200)
					.extract().response();
	...
	public void FISC_966_MS_Analyst_DC_attribute(){
		  ...
		  String entitiesURL =baseURI+"/v1/entities/1177812";
		  ...
		  Response res = given().header("Authorization", AuthrztionValue).header("X-App-Client-Id", XappClintIDvalue)
					...
					.statusCode(200)
					.body(containsString("analysts"))
					.body(containsString("disclosure"))
					.body(containsString("id"))
	...
	public void FISC_1134() throws IOException{
		...
		Response fieldResponse = given().header("Authorization", AuthrztionValue)
				...
				.body(containsString("FC_PEER_ISSUER_NAMES"))
				.body(containsString("China Merchants Bank"))
				.body(containsString("China Everbright Bank"))				
				.extract().response();	
	...
	public void FISC_965_transactionSecurityRatings_Relation(){
		  ...
		  Response res = given().header("Authorization", AuthrztionValue).header("X-App-Client-Id", XappClintIDvalue)
					.header("accept", acceptValue).header("content", contentValue).contentType("application/vnd.api+json")
					.when().get(transactionSecurityURi).then()
					.statusCode(200)
		  ...
		  String transactionSecurityRatingslink = res.path("data[0].relationships.transactionSecurityRatings.links.related");
		  
		  Response res1 = given().header("Authorization", AuthrztionValue).header("X-App-Client-Id", XappClintIDvalue)
					.header("accept", acceptValue).header("content", contentValue).contentType("application/vnd.api+json")
					.when().get(transactionSecurityRatingslink).then()
					.statusCode(200)
					.extract().response();
  ...
  String SingletransactionSecurityURi =baseURI+"/v1/transactionSecurities/96514261";
		  ...
		  Response res1 = given().header("Authorization", AuthrztionValue).header("X-App-Client-Id", XappClintIDvalue)
					.header("accept", acceptValue).header("content", contentValue).contentType("application/vnd.api+json")
					.when().get(SingletransactionSecurityURi).then()
					.statusCode(200)
					.body(containsString("transactionSecurities"))
					.body(containsString("description"))
					.body(containsString("agentStateName"))
	...
	public void FISC_958_TransactionSecurityRating () {
		 ...
		 String transactionSecurityURi =baseURI+"/v1/transactionSecurities/77663/transactionSecurityRatings?filter[action]=Revision Outlook&filter[alert]=Rating Outlook Stable&filter[startDate]=2010-01-01";
		  ...
		  Response res = given().header("Authorization", AuthrztionValue).header("X-App-Client-Id", XappClintIDvalue)
					...
					.body(containsString("transactionSecurity"))
					.extract().response();
