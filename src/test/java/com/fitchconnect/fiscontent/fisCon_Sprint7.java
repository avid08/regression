...
public class fisCon_Sprint7 extends Configuration {
	...
	public void fisc_604_FIR_entityRelationship() {
		...
		Response res = given().header("Authorization", (AuthrztionValue)).header("X-App-Client-Id", XappClintIDvalue)
				...
				.get(FIRgetAll).then().statusCode(200).body(containsString("statement"))
				.body(containsString("profitability")).body(containsString("fitchEntityId"))
		...
		Response res1 = given().header("Authorization", (AuthrztionValue)).header("X-App-Client-Id", XappClintIDvalue)
				.header("accept", acceptValue).header("content", contentValue).contentType(ContentType.JSON).when()
				.get(entiyRelationshiplink).then().statusCode(200).body(containsString("financialImpliedRatings"))
				.body(containsString("surveillanceDeals")).body(containsString("officers"))
				.body(containsString("statements")).extract().response();
	...
	public void fisc_605_FIR_statementRelation() {
		...
		Response res = given().header("Authorization", (AuthrztionValue)).header("X-App-Client-Id", XappClintIDvalue)
				.header("accept", acceptValue).header("content", contentValue).contentType(ContentType.JSON).when()
				.get(FIRgetAll).then().statusCode(200).body(containsString("statement")).extract().response();
		Assert.assertFalse(res.asString().contains("isError"));
		Assert.assertFalse(res.asString().contains("isMissing"));
		Assert.assertFalse(res.asString().contains("isRestricted"));
