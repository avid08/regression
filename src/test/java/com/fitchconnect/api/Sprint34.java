...
public void fca_1677() {
	...
	Response res = given().header("Authorization", AuthrztionValue).header("X-App-Client-Id", XappClintIDvalue)
			...
			.when().get(sandPratingsUrl).then().statusCode(200)
			.body(containsString("standardAndPoorIssuerRatings"))
			.body(containsString("Issuer Credit Rating"))
			.body(containsString("ratingDate"))
 ...
 public void fca_1678() {
		...
		Response res = given().header("Authorization", AuthrztionValue).header("X-App-Client-Id", XappClintIDvalue)
				.header("accept", acceptValue).header("content", contentValue).when().get(userUrl)
				.then().statusCode(200)
 ...
 public void BMI_Data_1620_Ratingfields_with_Xref_entity () throws IOException {
	 ...
	 Response res = given().header("Authorization", AuthrztionValue).header("X-App-Client-Id", XappClintIDvalue)
				.contentType("application/vnd.api+json").body(myJson).with().when().post(dataPostUrl).then()
				.assertThat().statusCode(200)
				.body(containsString("BMI_FISCAL_BALANCE_PCTGDP"))
				.body(containsString("FC_LT_IDR"))
				.body(containsString("BMI_GDP_NOM_LCU"))
				.body(containsString("ETB"))
				.body(containsString("Annual"))
				.body(containsString("value"))
				.body(containsString("currency"))
 ...
 public void FCA_1535() throws IOException {
        ...
        Response MSoutput = given()
                     .header("Authorization", AuthrztionValue).header("X-App-Client-Id", XappClintIDvalue)
                     .contentType("application/vnd.api+json").body(MSJson).with()
 ...
 public void FCA_1640_valuerequest() throws IOException {
        ...
        Response vroutput = given().header("Authorization", AuthrztionValue)
                     .header("X-App-Client-Id", XappClintIDvalue).contentType("application/vnd.api+json").body(vrJSON)
                     .with()
                     .when().post(dataPostUrl)
                     .then().assertThat().statusCode(200)
              .body(containsString("fitchid"))
                     .extract().response();
