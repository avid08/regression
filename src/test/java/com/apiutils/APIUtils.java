package com.apiutils;

import com.configuration.api.Configuration;
import com.google.common.io.Resources;
import com.jayway.restassured.response.Response;
import groovy.json.internal.Charsets;

import java.io.IOException;
import java.net.URL;

import static com.jayway.restassured.RestAssured.given;

public class APIUtils extends Configuration {

    public Response getResponse(String uri, String authValue, String xappClientIdValue, String acceptValue, String contentValue){
        return given()
                .header("Authorization", authValue)
                .header("X-App-Client-Id", xappClientIdValue)
                .header("accept", acceptValue)
                .header("Content", contentValue)
                .when().get(uri).then().statusCode(200)
                .extract().response();
    }

    public Response postToDataAggregator(String resourceFileName, String authValue, String xappClientIdValue, String dataPostUrl) throws IOException {

        URL file = Resources.getResource(resourceFileName);
        myjson = Resources.toString(file, Charsets.UTF_8);

        Response response = given()
                .header("Authorization", authValue)
                .header("X-App-Client-Id", xappClientIdValue)
                .contentType("application/vnd.api+json").body(myjson).with()
                .when().post(dataPostUrl).then().assertThat().statusCode(200)
                .extract()
                .response();
        return response;
    }

    public Response postToDataAggregatorBaseUser(String resourceFileName, String xappClientIdValue, String dataPostUrl) throws IOException {

        URL file = Resources.getResource(resourceFileName);
        myjson = Resources.toString(file, Charsets.UTF_8);

        Response response = given()
                .header("Authorization", "Bearer eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9..iuXBcwghr82ZYhIBq5Br5OJtH0RtswcWgjGnUXfqQ38")
                .header("X-App-Client-Id", xappClientIdValue)
                .contentType("application/vnd.api+json").body(myjson).with()
                .when().post(dataPostUrl).then().assertThat().statusCode(206)
                .extract()
                .response();
        return response;
    }
}
